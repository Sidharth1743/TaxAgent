...
import { ArrowUpIcon, SquareIcon, CameraIcon } from "lucide-react";
  ...
  return (
    <ChatInputContext.Provider value={contextValue}>
      <div
        className={cn(
          variant === "default" &&
            "flex flex-col items-end w-full p-2 rounded-2xl border border-border bg-card focus-within:border-ring focus-within:ring-1 focus-within:ring-ring/50 transition-all duration-200",
...
function ChatInputTextArea({
  ...
  return (
    <Textarea
      ...
      variant={variant}
      className={cn(
        "max-h-[200px] min-h-0 resize-none overflow-x-hidden text-sm leading-relaxed",
        className,
      )}
      rows={rows}
    />
...
interface ChatInputCameraProps {
  onCamera?: () => void;
  className?: string;
}

function ChatInputCamera({ onCamera, className }: ChatInputCameraProps) {
  return (
    <Button
      type="button"
      size="icon-sm"
...
ChatInputCamera.displayName = "ChatInputCamera";

export { ChatInput, ChatInputTextArea, ChatInputSubmit, ChatInputCamera };
