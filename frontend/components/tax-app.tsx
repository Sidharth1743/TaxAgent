...
export function TaxClarityApp() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onopen = () => {
      setConnected(true);
      setOrbState("idle");
      ws.send(JSON.stringify({ type: "config", user_id: userId.current }));
    };
  ...
  const handleServerMessage = useCallback((msg: any) => {
    switch (msg.type) {
      ...
      case "tool_result":
        if (msg.result) {
          const newSources = extractSources(msg.result);
          if (newSources.length) {
            setSources((prev) => [...prev, ...newSources]);
          }
          if (msg.result.claims && currentAgentMsgIdRef.current) {
            const urls = (msg.result.claims as any[]).flatMap((c: any) => c.citations || []);
            if (urls.length) {
              setMessages((prev) =>
                prev.map((m) =>
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const ws = wsRef.current;
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      ws.send(JSON.stringify({ type: "image", data: b64, mime_type: file.type || "image/jpeg" }));
      ws.send(JSON.stringify({ type: "text", content: `I've uploaded a document (${file.name}). Please analyze it.` }));
