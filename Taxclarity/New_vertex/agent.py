...
def _run_async(coro):
    ...
    def _target() -> None:
        ...
        except Exception as exc:  # pragma: no cover - defensive bridge for sync ADK tool use
            error["exc"] = exc
...
def get_memory_context_tool(query: str, user_id: str) -> Dict[str, Any]:
    try:
        context = _run_async(get_memory_service().load_conversation_context(user_id=user_id)) or {}
        return {
            "summary": context.get("summary", ""),
            "prior_resolutions": context.get("recent_turns", []) or [],
            "unresolved_queries": context.get("top_topics", []) or context.get("prior_topics", []) or [],
            "loaded": bool(context.get("loaded")),
        }
    except Exception:
        return {"summary": "", "prior_resolutions": [], "unresolved_queries": [], "loaded": False}
    ...
    try:
        ...
        try:
            ...
            if isinstance(parsed, dict):
                answer_text = (
                    parsed.get("message")
                    or parsed.get("synthesized_response")
        ...
        _run_async(service.enqueue_summary_refresh(user_id=user_id, session_id=session_id))
        return {"ok": True}
