...
def _adapt_memory_context(context: dict[str, Any]) -> dict[str, Any]:
    recent_turns = context.get("recent_turns", []) or []
    top_topics = context.get("top_topics", []) or context.get("prior_topics", []) or []
    vertex_memories = context.get("vertex_memories", []) or []
    prior_resolutions = [
        {
            "query": turn.get("text", ""),
            "status": turn.get("role", ""),
    ...
    return {
        ...
        "loaded": bool(context.get("loaded") or recent_turns or top_topics or vertex_memories),
        "prior_resolutions": prior_resolutions,
        "unresolved_queries": unresolved_queries,
    }


async def _load_memory_context(user_id: str) -> dict[str, Any]:
    try:
        context = await get_memory_service().load_conversation_context(user_id=user_id)
        return _adapt_memory_context(context)
    except Exception as exc:
        logger.warning("memory_context_fetch_failed", error=str(exc), user_id=user_id)
        return _adapt_memory_context({})


async def _persist_memory(query: str, user_id: str, session_id: str, answer_text: str) -> None:
    memory_service = get_memory_service()
    try:
        user_turn_id = await memory_service.append_turn(
            user_id=user_id,
            session_id=session_id,
            role="user",
            text=query,
        )
        await memory_service.enqueue_turn_memory(
            user_id=user_id,
            session_id=session_id,
            role="user",
            text=query,
        ...
        agent_turn_id = await memory_service.append_turn(
            user_id=user_id,
            session_id=session_id,
            role="agent",
            text=answer_text,
        ...
        await memory_service.enqueue_summary_refresh(user_id=user_id, session_id=session_id)
    except Exception as exc:
        logger.warning(
            "live_memory_persist_failed",
            error=str(exc),
            user_id=user_id,
            session_id=session_id,
        )
...
async def run_live_query(query: str, user_id: str, session_id: str) -> dict[str, Any]:
    ...
    memory_context = await _maybe_await(_load_memory_context(user_id))
    ...
    if claims:
        await _maybe_await(_persist_memory(query, user_id, session_id, synthesized))
